...
class PathSelector:
    def __init__(self):
        ...
        if "sim_videos" not in st.session_state:
            st.session_state.sim_videos = {}
        ...
        # **Tombol Refresh untuk memperbarui status & video**
        if st.button("Refresh Status & Video"):
            st.rerun()
        ...
        for i in range(st.session_state.num_sim):
            with st.container():
                ...
                # **Tempat untuk status dan video hasil simulasi**
                ...
                if st.button(label=f"START SIMULATION {i+1}", key=f"submit_button_{i+1}"):
                    # **Hapus video sebelumnya sebelum simulasi baru dimulai**
                    st.session_state.pop(f"sim_video_{i+1}", None)
                ...
                # **Tampilkan video hasil simulasi di dalam container terkait**
                if f"sim_video_{i+1}" in st.session_state:
                    video_path = st.session_state[f"sim_video_{i+1}"]
                    if video_path and os.path.exists(video_path):
                        video_placeholder.video(video_path)

    def run_simulation(self, simulation_id):
        """Menjalankan simulasi di subprocess"""
        video_path = self.execSimulation(simulation_id)

        # **Simpan path video ke session state**
        if video_path:
            st.session_state[f"sim_video_{simulation_id}"] = video_path
    ...
    def execSimulation(self, simulation_id):
        ...
        try:
            ...
            # **Ambil video hasil simulasi**
            time.sleep(1)  # Beri waktu agar file tersimpan
            files = sorted(
                [f for f in os.listdir(SIMULATION_FOLDER) if f.endswith(".mp4")],  # Ubah ke format video
                key=lambda x: os.path.getmtime(os.path.join(SIMULATION_FOLDER, x)),
                reverse=True
            )

            if files:
                video_path = os.path.join(SIMULATION_FOLDER, files[0])
                print(f"Simulation {simulation_id} End! Video saved at {video_path}")
                return video_path
            else:
                print(f"Simulation {simulation_id} End! No video found.")
                return None
